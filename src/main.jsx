...
export const products=[
 {id:1,name:'AMD Ryzen 7 7800X3D',cat:'CPU',brand:'AMD',price:9890000,old:10990000,rate:4.9,reviews:128,badge:'-10%',img:'https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?auto=format&fit=crop&w=700&q=80',description:'Bộ xử lý gaming hiệu năng cao với công nghệ AMD 3D V-Cache.',productInfo:'AMD Ryzen 7 7800X3D là bộ xử lý được tối ưu cho gaming, sử dụng kiến trúc Zen 4 và công nghệ 3D V-Cache. Sản phẩm mang lại hiệu năng cao, mức tiêu thụ điện hợp lý và hỗ trợ nền tảng AM5 hiện đại.',attributes:[{name:'Số nhân',value:'8',unit:'nhân'},{name:'Số luồng',value:'16',unit:'luồng'},{name:'Xung nhịp tối đa',value:'5.0',unit:'GHz'},{name:'Bộ nhớ đệm',value:'96',unit:'MB'},{name:'Socket',value:'AM5'},{name:'Thế hệ RAM hỗ trợ',value:'DDR5'},{name:'Công suất',value:'120',unit:'W'}]},
 {id:2,name:'GeForce RTX 4070 SUPER',cat:'VGA',brand:'GIGABYTE',price:17890000,old:19590000,rate:4.8,reviews:86,badge:'BÁN CHẠY',img:'https://images.unsplash.com/photo-1591488320449-011701bb6704?auto=format&fit=crop&w=700&q=80',attributes:[{name:'Công suất',value:'220',unit:'W'}]},
 {id:3,name:'ROG STRIX B650E-F Gaming WiFi',cat:'Mainboard',brand:'ASUS',price:6590000,old:7290000,rate:4.7,reviews:54,badge:'-9%',img:'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=700&q=80',attributes:[{name:'Socket',value:'AM5'},{name:'Khe RAM',value:'DDR5'},{name:'Kích thước',value:'ATX'},{name:'Công suất',value:'50',unit:'W'}]},
 {id:4,name:'Corsair Vengeance RGB 32GB DDR5',cat:'RAM',brand:'CORSAIR',price:2890000,old:3290000,rate:4.9,reviews:102,badge:'MỚI',img:'https://images.unsplash.com/photo-1562976540-1502c2145186?auto=format&fit=crop&w=700&q=80',attributes:[{name:'Thế hệ RAM',value:'DDR5'},{name:'Công suất',value:'10',unit:'W'}]},
 {id:5,name:'Samsung 990 PRO 2TB NVMe',cat:'SSD',brand:'SAMSUNG',price:4190000,old:4690000,rate:4.8,reviews:74,badge:'-11%',img:'https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?auto=format&fit=crop&w=700&q=80',attributes:[{name:'Công suất',value:'8',unit:'W'}]},
 {id:6,name:'Cooler Master MWE Gold 850 V2',cat:'Nguồn',brand:'COOLER MASTER',price:2490000,old:2790000,rate:4.6,reviews:39,badge:'GIÁ TỐT',img:'https://images.unsplash.com/photo-1587202372775-e229f172b9d7?auto=format&fit=crop&w=700&q=80',attributes:[{name:'Công suất',value:'850',unit:'W'}]},
 {id:7,name:'NZXT H510 Flow',cat:'Case',brand:'NZXT',price:1990000,old:2290000,rate:4.7,reviews:61,badge:'MỚI',img:'https://images.unsplash.com/photo-1587202372775-e229f172b9d7?auto=format&fit=crop&w=700&q=80',attributes:[{name:'Kích thước hỗ trợ',value:'ATX, Micro-ATX, Mini-ITX'}]},
];
function App(){
 ...
 const feedbacks=[
  {name:'Minh Khoa',place:'Build PC Gaming • TP.HCM',text:'“Build PC qua AI rất nhanh, linh kiện tương thích hết. Giao hàng nội thành đúng 2 giờ như cam kết.”',img:'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=100&q=80'},
  {name:'Thu Hà',place:'PC đồ họa • Hà Nội',text:'“Nhân viên hỗ trợ chọn cấu hình theo ngân sách rất tận tâm. Lắp ráp miễn phí, đi dây gọn đẹp.”',img:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=100&q=80'},
  {name:'Đức Anh',place:'Nâng cấp linh kiện • Đà Nẵng',text:'“Mua SSD flash sale giá tốt, bảo hành chính hãng rõ ràng. Đổi mới trong 30 ngày cũng yên tâm.”',img:'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=100&q=80'},
  {name:'Quốc Bảo',place:'Streamer • Cần Thơ',text:'“RTX mua tại TechZone chạy ổn định, chat AI tư vấn cấu hình livestream rất chuẩn.”',img:'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=100&q=80'},
  {name:'Lan Anh',place:'Văn phòng • Bình Dương',text:'“PC văn phòng lắp sẵn, giao nhanh, bảo hành rõ ràng. Sẽ quay lại nâng cấp thêm RAM.”',img:'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&w=100&q=80'},
 ];
 return <SiteLayout products={products}>{({add,openBuilder})=>
  <main>
   <section className="home-hero"><aside className="hero-catalog"><h2><Menu/> Danh mục sản phẩm</h2>{cats.map(([name,Icon])=><a key={name} href={`/collections/${categorySlug(name)}`}><Icon/><span>{name}</span></a>)}<a href="/collections/all"><Box/><span>Xem tất cả sản phẩm</span></a></aside><div className="hero-showcase"><div className="hero-main"><img src="/pc-hero.png" alt="Build PC cùng TechZone"/><div className="hero-main-copy"><span><Sparkles/> TƯ VẤN CẤU HÌNH BẰNG AI</span><h1>BUILD PC<br/><strong>KHÔNG CÒN KHÓ</strong></h1><p>Chọn đúng linh kiện, tối ưu hiệu năng<br/>và kiểm tra tương thích trong vài giây.</p><button onClick={openBuilder}>BUILD PC NGAY <ArrowRight/></button></div></div><div className="hero-deals"><article><div><span>COMBO TIẾT KIỆM</span><h3>CPU + Mainboard</h3><p>Hiệu năng mạnh mẽ, giá tốt hơn khi mua cùng.</p><button onClick={()=>location.href='/collections/cpu'}>Xem combo <ChevronRight/></button></div><img src={products[0].img} alt="CPU combo"/></article><article><div><span>PC GAMING</span><h3>Chiến game cực đỉnh</h3><p>Linh kiện cân mọi tựa game phổ biến.</p><button onClick={openBuilder}>Build cùng AI <ChevronRight/></button></div><img src={products[1].img} alt="Gaming graphics card"/></article><article className="deal-accent"><div><span>FLASH SALE</span><h3>SSD tốc độ cao</h3><p>Giảm đến 11% trong hôm nay.</p><button onClick={()=>location.href='/collections/ssd'}>Mua ngay <ChevronRight/></button></div><img src={products[4].img} alt="SSD sale"/></article></div></div></section>
   <HomePromotions products={catalog} add={add} openBuilder={openBuilder}/>
   ...
   <section className="shop"><div className="section-title"><div><span>ƯU ĐÃI HÔM NAY</span><h2>Sản phẩm nổi bật</h2></div><div className="tabs">{['Tất cả','CPU','VGA','Mainboard','RAM'].map(x=><button key={x} className={active===x?'on':''} onClick={()=>setActive(x)}>{x}</button>)}</div></div>
    ...
    <div className="testimonial-rail-wrap">
     <div className="testimonial-rail" ref={feedbackRail}>
      {feedbacks.map(f=><article className="testimonial-card" key={f.name}>
       ...
       <div className="testimonial-author"><img src={f.img} alt=""/><div><b>{f.name}</b><small>{f.place}</small></div></div>
      </article>)}
     </div>
    </div>
   </section>
...
function CollectionPage(){
 ...
 const catalogAll=readStore('techzone_products',products).filter(p=>p.active!==false&&(category==='Tất cả'||p.cat===category));
 const[added,setAdded]=useState([]);
 const add=p=>{addToStoredCart(p);setAdded(ids=>ids.includes(p.id)?ids:[...ids,p.id])};
 if(!category)return <SiteLayout products={products}><main className="collection-page"><div className="checkout-empty"><Box/><h2>Không tìm thấy danh mục</h2><a href="/">Về trang chủ</a></div></main></SiteLayout>;
 return <SiteLayout products={products}><main className="collection-page"><div className="product-breadcrumb"><a href="/">Trang chủ</a><ChevronRight/><b>{category}</b></div><div className="collection-heading"><div><span>COLLECTION</span><h1>{category==='Tất cả'?'Tất cả sản phẩm':category}</h1><p>{catalogAll.length} sản phẩm đang hiển thị</p></div><a href="/">← Về trang chủ</a></div>{catalogAll.length?<div className="product-grid collection-grid">{catalogAll.map(p=><article key={p.id} className="product-card-link" role="link" tabIndex={0} onClick={()=>location.href=`/product/${p.id}`} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();location.href=`/product/${p.id}`}}}><a className="visual product-link" href={`/product/${p.id}`}><img src={p.img} alt={p.name}/>{p.badge&&<span className={p.badge==='BÁN CHẠY'?'hot':''}>{p.badge}</span>}</a><small>{p.brand} • {p.cat}</small><h3><a className="product-name" href={`/product/${p.id}`}>{p.name}</a></h3><div className="rating"><Star fill="currentColor"/> {p.rate||5} <span>({p.reviews||0} đánh giá)</span></div><div className="price"><b>{money(p.price)}</b>{p.old>p.price&&<del>{money(p.old)}</del>}</div><button className={`add ${added.includes(p.id)?'added':''}`} onClick={e=>{e.stopPropagation();add(p)}}>{added.includes(p.id)?<><Check/> Đã thêm vào giỏ</>:<><ShoppingCart/> Thêm vào giỏ</>}</button></article>)}</div>:<div className="collection-empty"><Box/><h2>Danh mục chưa có sản phẩm</h2><p>Sản phẩm mới sẽ sớm được cập nhật.</p></div>}</main></SiteLayout>;
}
function ProductDetailsPage(){
 ...
 if(!product)return <SiteLayout products={products}><main className="product-page"><div className="product-not-found"><h1>Không tìm thấy sản phẩm</h1><p>Sản phẩm có thể đã bị xóa hoặc đường dẫn không còn hợp lệ.</p><a href="/">← Quay lại cửa hàng</a></div></main></SiteLayout>;
...
}function OrderDetailPage(){const code=decodeURIComponent(location.pathname.split('/').filter(Boolean)[1]||''), {orders,syncError}=useCustomerOrders(code),order=orders.find(o=>o.code===code);if(!order)return <ConsumerHeader><main className="orders-page"><div className="checkout-empty"><WarningIcon/><h2>Không tìm thấy đơn hàng</h2><a href="/orders">Xem lịch sử đơn</a></div></main></ConsumerHeader>;return <ConsumerHeader><main className="orders-page"><div className="checkout-heading"><a href="/orders">← Danh sách đơn hàng</a><h1>Đơn hàng #{order.code}</h1><p>Đặt lúc {new Date(order.createdAt).toLocaleString('vi-VN')}</p></div><div className="order-detail-grid"><div><section className="order-status"><h2>{orderLabel(order.status)}</h2><p>{order.paymentMethod==='COD'?'Thanh toán khi nhận hàng':'Thanh toán điện tử Sandbox'} • {order.paymentStatus==='PAID'?'Đã thanh toán':'Chưa thanh toán'}</p>{syncError&&<p className="checkout-error" role="status">Chưa thể đồng bộ trạng thái mới nhất. Hệ thống sẽ tự thử lại.</p>}<OrderProgress order={order}/></section><section className="ordered-products"><h2>Sản phẩm</h2>{order.items.map(x=><div key={x.sku}><img src={x.img}/><span><b>{x.name}</b><small>SKU: {x.sku} • SL: {x.quantity}</small></span><strong>{money(x.total)}</strong></div>)}</section></div><aside className="order-meta"><section><h3>Địa chỉ nhận hàng</h3><b>{order.address.name}</b><p>{order.address.phone}<br/>{order.address.address}, {order.address.province}</p></section><section><h3>Thanh toán</h3><span>Tạm tính <b>{money(order.totals.subtotal)}</b></span><span>Phí giao hàng <b>{order.totals.shipping?money(order.totals.shipping):'Miễn phí'}</b></span><strong>Tổng cộng <b>{money(order.totals.grandTotal)}</b></strong></section></aside></div></main></ConsumerHeader>}
...
function AddressBookPage(){
 ...
 </main>{modal&&<AddressFormModal initial={modal==='new'?null:modal} onClose={()=>setModal(null)} onSave={save}/>}</ConsumerHeader>
}
function ConsumerHeader({children,showFooter=false}){return <SiteLayout products={products} showFooter={showFooter}>{children}</SiteLayout>}
function CategoryPage({title,description,icon:Icon,filterCat}){
 const catalog=readStore('techzone_products',products).filter(p=>p.active!==false&&(filterCat?p.cat===filterCat:true));
 const[added,setAdded]=useState([]);
 const add=p=>{addToStoredCart(p);setAdded(ids=>ids.includes(p.id)?ids:[...ids,p.id])};
 return <SiteLayout products={products}><main className="category-page"><div className="category-hero"><div className="category-hero-content"><Icon size={48}/><h1>{title}</h1><p>{description}</p><span>{catalog.length} sản phẩm</span></div></div><div className="category-products"><div className="product-grid">{catalog.map(p=><article key={p.id} className="product-card-link" role="link" tabIndex={0} onClick={()=>location.href=`/product/${p.id}`} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();location.href=`/product/${p.id}`}}}><a className="visual product-link" href={`/product/${p.id}`}><img src={p.img} alt={p.name}/>{p.badge&&<span className={p.badge==='BÁN CHẠY'?'hot':''}>{p.badge}</span>}</a><small>{p.brand} • {p.cat}</small><h3><a className="product-name" href={`/product/${p.id}`}>{p.name}</a></h3><div className="rating"><Star fill="currentColor"/> {p.rate||5} <span>({p.reviews||0} đánh giá)</span></div><div className="price"><b>{money(p.price)}</b>{p.old>p.price&&<del>{money(p.old)}</del>}</div><button className={`add ${added.includes(p.id)?'added':''}`} onClick={e=>{e.stopPropagation();add(p)}}>{added.includes(p.id)?<><Check/> Đã thêm vào giỏ</>:<><ShoppingCart/> Thêm vào giỏ</>}</button></article>)}</div></div></main></SiteLayout>;
}
function PcGamingPage(){return <CategoryPage title="PC GAMING" description="Linh kiện gaming cao cấp cho trải nghiệm chơi game đỉnh cao" icon={Gamepad} filterCat="VGA"/>;}
function LaptopPage(){return <CategoryPage title="LAPTOP" description="Các dòng laptop từ văn phòng đến gaming, đáp ứng mọi nhu cầu" icon={Laptop}/>;}
function MonitorPage(){return <CategoryPage title="MÀN HÌNH" description="Màn hình chất lượng cao cho gaming và thiết kế chuyên nghiệp" icon={Monitor}/>;}
function AccessoriesPage(){return <CategoryPage title="PHỤ KIỆN" description="Phụ kiện và thiết bị ngoại vi hoàn thiện hệ thống của bạn" icon={Wrench}/>;}
function TechNewsPage(){
 return <SiteLayout products={products}><main className="tech-news-page"><div className="category-hero"><div className="category-hero-content"><Newspaper size={48}/><h1>TIN CÔNG NGHỆ</h1><p>Cập nhật tin tức công nghệ mới nhất từ thế giới hardware và gaming</p></div></div><div className="news-grid"><article className="news-card"><div className="news-image"><img src={products[0].img} alt="News"/></div><div className="news-content"><span className="news-tag">HARDWARE</span><h3>Công nghệ chip 3nm mới nhất từ AMD và Intel</h3><p>Khám phá các tiến bộ trong quy trình sản xuất chip và hiệu năng mang lại.</p><a href="#" className="read-more">Đọc thêm →</a></div></article><article className="news-card"><div className="news-image"><img src={products[1].img} alt="News"/></div><div className="news-content"><span className="news-tag">GRAPHICS</span><h3>RTX 50 Series: Sự đột phá trong đồ họa</h3><p>NVIDIA chuẩn bị ra mắt dòng card đồ họa mới với hiệu năng vượt trội.</p><a href="#" className="read-more">Đọc thêm →</a></div></article><article className="news-card"><div className="news-image"><img src={products[4].img} alt="News"/></div><div className="news-content"><span className="news-tag">STORAGE</span><h3>SSD Gen5: Tốc độ kỷ lục mới</h3><p>Thế hệ SSD mới với tốc độ đọc ghi lên đến 14GB/s.</p><a href="#" className="read-more">Đọc thêm →</a></div></article></div></main></SiteLayout>;
}
const path=location.pathname;
createRoot(document.getElementById('root')).render(<ConfigProvider locale={viVN} theme={{token:{fontFamily:"Arial, sans-serif"}}}>{path.startsWith('/admin')?<React.Suspense fallback={<div style={{padding:40,fontFamily:'sans-serif'}}>Đang tải trang quản trị...</div>}><AdminApp seed={products}/></React.Suspense>:path==='/register'?<RegisterPage/>:path==='/login'?<LoginPage/>:path==='/checkout'?<CheckoutPage/>:path==='/orders'?<OrdersPage/>:path==='/addresses'?<AddressBookPage/>:path==='/profile'?<ProfilePage/>:path==='/pc-gaming'?<PcGamingPage/>:path==='/laptop'?<LaptopPage/>:path==='/monitor'?<MonitorPage/>:path==='/accessories'?<AccessoriesPage/>:path==='/tech-news'?<TechNewsPage/>:path.startsWith('/orders/')?<OrderDetailPage/>:path.startsWith('/collections/')?<CollectionPage/>:path.startsWith('/product/')?<ProductDetailsPage/>:<App/>}{!path.startsWith('/admin') && !['/login','/register'].includes(path) && <Chatbot/>}</ConfigProvider>);
